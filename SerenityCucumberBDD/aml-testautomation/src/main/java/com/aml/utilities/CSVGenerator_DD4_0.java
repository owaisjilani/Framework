...
public class CSVGenerator_DD4_0{
		...
		public static List<String> countryCode = asList("US","AE","AF","CA","IN","IT","CU","PK","UN","MP","MH","TJ","OM","SA","SE","UA","CK","QA","RO","ZW","ID");
		public static List<String> country = asList("United States of America");
		public static String DATE_TIME_FORMAT = "yyyy-MM-dd HH:mm:ss";
		public static DateTimeFormatter dateTimeFormatter = ofPattern(DATE_TIME_FORMAT);
	    ...
	    private static List<String> riskLevel = asList("HIGH", "MEDIUM","LOW");
	    public static List<String> multipleReviewCycle = asList("56-D","30-D", "4-Y","67-D" ,"1-Y", "3-Y", "12-M", "6-M","24-D","4-W","5-Y","2-W","9-W");
	    public static List<String> teamCode = asList("OS007", "72-AR", "IND-67", "YOLO", "SQUAD-2","QA-OWASP");
	    public static List<String> multipleRiskFactors = asList("Country Risk; Citizenship Risk","Country Risk","Citizenship Risk","Geographic Risk;PEP indicator Risk","Product Risk; CTR Exemption indicator; High-Risk Entities Risk indicator","Transactional Risk; Relationship Length Risk; SAR Filing Risk; ","Occupation Risk","NAICS Code Risk");
		...
		static public void  generateAllCSVs(int noOfRecords,int individualStartRange,int individualAccStartRange,int businessStartRange,int businessAccStartRange) {	         
			...
			createExternalCDDCSV(noOfRecords,individualStartRange,businessStartRange);
			createACCOUNTRELATIONSHIP_DELETECSV(noOfRecords,individualAccStartRange,businessAccStartRange);
	...
	// New CSV File added in Data Dictionary V4.0 - (serenity-16225)
	static private void createExternalCDDCSV(int noOfRecords,int individualStartRange,int businessStartRange) {
	       csvs()
	           .column("SUBJECT_ID")
	           .column("ALLOW_XCEED_TO_OVERRIDE_RISK_LEVEL")
	           .column("EXTERNAL_RISK_LEVEL")
	           .column("ALLOW_XCEED_TO_OVERRIDE_RISK_SCORE")
	           .column("EXTERNAL_RISK_SCORE")
	           .column("EXTERNAL_RISK_UPDATED_ON")
	           .column("EXTERNAL_NEXT_REVIEW_DATE")
	           .column("EXTERNAL_REVIEW_CYCLE")
	           .column("EXTERNAL_RISK_FACTORS")
	           .write("DataIntakeInput/EXTERNALCDD_.csv", 1, false);
	       csvs()
	           .column(m.intSeq().start(individualStartRange).increment(1))
	           .column(fromStrings(booleanValues))
	           .column(fromStrings(riskLevel))
	           .column(fromStrings(booleanValues))
	           .column(ints().rangeClosed(1,1000))
	           .column(LocalDateTime.now().format(dateTimeFormatter))
	           .column("2024-04-24 12:30:56")
	           .column(fromStrings(multipleReviewCycle)) 
	           .column(fromStrings(multipleRiskFactors)) 
	           .write("DataIntakeInput/EXTERNALCDD_.csv", noOfRecords, true);
	       csvs()
        .column(m.intSeq().start(businessStartRange).increment(1))
        .column(fromStrings(booleanValues))
        .column(fromStrings(riskLevel))
        .column(fromStrings(booleanValues))
        .column(ints().rangeClosed(1,999))
        .column(LocalDateTime.now().format(dateTimeFormatter))
        .column("2024-10-08 17:08:06")
        .column(fromStrings(multipleReviewCycle)) 
        .column(fromStrings(multipleRiskFactors))
        .write("DataIntakeInput/EXTERNALCDD_.csv", noOfRecords, true);      
	       System.out.format("\nEXTERNALCDD CSV File Generated Successfully...");
	   }
	
	// New CSV File added in Data Dictionary V4.0 - (serenity-16352)
		static private void createACCOUNTRELATIONSHIP_DELETECSV(int noOfRecords,int individualAccStartRange, int businessAccStartRange) {
		       csvs()
		           .column("ACCOUNT_NUMBER")
		           .column("CUSTOMER_ID")
		           .column("RELATIONSHIP_TYPE")
		           .write("DataIntakeInput/ACCOUNTRELATIONSHIP_DELETE_.csv", 1, false);
		       csvs()
		           .column(000)
		           .column("NULL")
		           .column("NULL")
		           
		           .write("DataIntakeInput/ACCOUNTRELATIONSHIP_DELETE_.csv", noOfRecords, true);
		       csvs()
		       .column(000)
	           .column("NULL")
	           .column("NULL")
	        .write("DataIntakeInput/ACCOUNTRELATIONSHIP_DELETE_.csv", noOfRecords, true);      
		       System.out.format("\nACCOUNTRELATIONSHIP_DELETE CSV File Generated Successfully...");
		   }
}
